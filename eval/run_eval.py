"""
Evaluation harness (spec section 6).
...
Computes and prints:
  - conflict detection rate (recall on deliberately-conflicting questions,
    false-positive rate on deliberately-non-conflicting questions)
  - retry trigger rate
  - citation marker presence rate (a proxy for citation coverage; true
    "citation accuracy" per spec section 6 requires a human to sample and
    check each citation actually supports its adjacent claim - this harness
    reports the automatable proxy and flags that the human-graded number is
    a separate, required step, not something this script can honestly claim
    to compute)

IMPORTANT HONESTY NOTE: this repository was scaffolded in an environment
with no network access and no installable packages, so the numbers in the
shipped README were NOT generated by an actual run of this script against
real or even mock providers. Run this script yourself after
`pip install -r requirements.txt` to get real numbers before publishing them
anywhere. Do not copy placeholder numbers into a portfolio README.
"""
